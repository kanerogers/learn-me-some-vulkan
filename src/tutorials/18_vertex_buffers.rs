...
impl HelloTriangleApplication {
    ...
    pub fn recreate_swap_chain(&mut self, window: &Window) {
        unsafe { self.device.device_wait_idle().expect("Could not wait idle") };
        unsafe { self.cleanup_swap_chain() };

        let (swap_chain_ext, swap_chain, format, extent) = create_swap_chain(&self.instance, &self.entry, self.physical_device, self.surface, window, &self.device);
        self.swap_chain = swap_chain;
        let mut swap_chain_images = get_swap_chain_images(&self.instance, &self.device, swap_chain);
        self.swap_chain_image_views = create_image_views(&mut swap_chain_images, format, &self.device);
        self.render_pass = create_render_pass(format, &self.device);
        let (pipeline_layout, pipeline) = create_graphics_pipeline(&self.device, extent, self.render_pass);
    ...
    pub fn draw_frame(&mut self, window: &Window) {
        ...
        unsafe { self.device.queue_submit(self.graphics_queue, &submits, *fence).expect("Unable to submit to queue") };
