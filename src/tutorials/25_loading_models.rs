...
impl HelloTriangleApplication {
    ...
    pub fn recreate_swap_chain(&mut self, window: &Window) {
        ...
        let (depth_image, depth_image_memory) =
            create_depth_image(&self.context, self.swap_chain.extent);
        self.depth_image = depth_image;
        self.depth_image_memory = depth_image_memory;
        self.depth_image_view = create_depth_image_view(&self.context, depth_image);
...
fn find_depth_format(context: &VulkanContext) -> vk::Format {
    ...
    context.find_supported_format(candidates, tiling, features)
}
...
fn create_command_buffers(
        ...
        izip!(&command_buffers, swap_chain_framebuffers, descriptor_sets)
    {
        ...
        let clear_colors = [clear_color, clear_depth];
